defmodule Elkrotik.Api do

  @eof ""

  alias Elkrotik.{Utils}
  ...
  def read_block(pid, block \\ []) do
    case read_sentence(pid) do
      {false, sentence } -> read_block( pid, [ sentence | block])
      {_result, sentence} ->
  ...
  def read_sentence( pid, acc, spec) do
    ...
    case word do
      ...
      '!done' -> read_sentence(pid, [word | acc], :done)
      '!trap' -> read_sentence(pid, [word | acc], :trap)
      '!fatal' -> read_sentence(pid, [word | acc], :fatal)
